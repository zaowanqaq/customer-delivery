            ...
            try:
                ...
                if not note_detail:
                    note_detail = await self.xhs_client.get_note_by_id_from_html(note_id, xsec_source, xsec_token,
                                                                                 enable_cookie=True)
                    if not note_detail:
                        utils.logger.warning(f"[get_note_detail_async_task] Note detail unavailable (skipped), Id: {note_id}")
                        return None
                ...
                utils.logger.error(f"[get_note_detail_async_task] Unexpected error for note_id:{note_id}, err: {ex}")
                return None
